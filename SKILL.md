...
**⭐ 默认一次生成整套交付物 = 4 页报告 + 定妆照(3 场景) + 证件照(2 版)**。
不要只给 4 页就停——除非用户明确说「只要报告」，否则定妆照和证件照是**标配**，自动一起出。

> 🅐 **Codex 等有内置生图的环境**：不跑下面脚本，改读 [`assets/prompts.md`](assets/prompts.md)，对**全部产物**（P1-P4 + 定妆照各场景 + 证件照）逐一填占位符 + 用照片做输入图 + 调内置生图。
> 🅑 **Claude Code 等无内置生图**：一键脚本全出 ↓

```bash
node ${SKILL_DIR}/scripts/generate-all.mjs \
  --data analysis.json --photo 原始照片.jpg --out ~/Desktop/色彩报告/昵称
# 默认：报告4页 + 定妆照(樱花/影楼/咖啡馆) + 证件照×2。可加 --scenes / --idcount / --skip 调整
```

> 生成耗时较长（每张 1–3 分钟，整套约 15–25 分钟）。**开跑前先告诉用户「整套约需 X 分钟，我会陆续发给你」**，避免久等无反馈。可后台跑、分批回图。
> 单独重跑某类用：`generate-report.mjs` / `generate-portrait.mjs` / `generate-idphoto.mjs`（见 Step 5）。
...
### Step 5：单独重跑 / 追加（定妆照、证件照已在 Step 3 默认生成）

需要换场景、换背景色、多出几版时，单独跑：

**定妆照**（场景）：
```bash
node ${SKILL_DIR}/scripts/generate-portrait.mjs --data analysis.json --photo 原始照片.jpg \
  --out 目录 --scene sakura,studio,cafe   # 还有 hanok/city/garden
```
**韩式半身证件照**（背景随季型）：
```bash
...
**交付**：Read 展示**全部成品**（报告 4 页 + 定妆照各场景 + 证件照各版）；给 3–5 句口头总结（季型 + 2 个穿衣结论 + 1 个踩雷）；附一段可直接发的小红书文案（标题 + 话题 #我的四季型Note #色彩测试 #个人形象）。
